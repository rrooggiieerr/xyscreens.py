...
class TestXYScreens(TestCase):
    ...
    def test_constructor(self):
        screen = XYScreens(self._url, self.address, 60)
        self.assertIsNotNone(screen)

    def test_constructor2(self):
        screen = XYScreens(self._url, self.address, 60, 60)
        self.assertIsNotNone(screen)

    def test_constructor3(self):
        screen = XYScreens(self._url, self.address, 60, 60, 100.0)
        self.assertIsNotNone(screen)

    def test_constructor_up(self):
        screen = XYScreens(self._url, self.address, 60, position=0.0)
        self.assertIs(XYScreensState.UP, screen.state())
        self.assertIs(0.0, screen.position())

    def test_constructor_down(self):
        screen = XYScreens(self._url, self.address, 60, position=100.0)
        self.assertIs(XYScreensState.DOWN, screen.state())
        self.assertIs(100.0, screen.position())

    def test_constructor_stopped(self):
        screen = XYScreens(self._url, self.address, 60, position=50.0)
    ...
    def test_up(self):
        screen = XYScreens(self._url, self.address, 60, 60, 100)
    ...
    def test_state_up(self):
        screen = XYScreens(self._url, self.address, 10, 10, 100)
        screen.up()
        time.sleep(10)
        self.assertIs(XYScreensState.UP, screen.state())
    ...
    def test_state_stopped(self):
        screen = XYScreens(self._url, self.address, 10, 10)
        screen.down()
        time.sleep(5)
        screen.stop()
        self.assertIs(XYScreensState.STOPPED, screen.state())
    ...
    def test_set_position_upward(self):
        screen = XYScreens(self._url, self.address, 10, 10, 100.0)
        screen.set_position(50.0)
        state, position = screen.update_status()
        self.assertIs(XYScreensState.STOPPED, state)
        self.assertAlmostEqual(50.0, position, delta=1)
